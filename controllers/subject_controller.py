...
class SubjectController:
    ...
    def run(self):
        while True:
            choice = input(f"{INDENT}Student Course Menu (c/e/r/s/x): ").strip().lower()
            if choice == 'c':
    ...
    def _enrol(self):
        if len(self.student.subjects) >= MAX_SUBJECTS:
            print(f"{INDENT}Students are allowed to enrol in 4 subjects only")
            return
        subject = Subject()
        # ensure unique ID within student's subjects
        existing_ids = {s.id for s in self.student.subjects}
        while subject.id in existing_ids:
            subject = Subject()
        self.student.subjects.append(subject)
        print(f"{INDENT}Enrolling in Subject-{int(subject.id)}")
        print(f"{INDENT}You are now enrolled in {len(self.student.subjects)} out of 4 subjects")
        self._save()

    def _remove_subject(self):
        subject_id = input(f"{INDENT}Remove Subject by ID: ").strip()
        ...
        for subject in self.student.subjects:
            if subject.id == subject_id_padded:
                ...
                print(f"{INDENT}Droping Subject-{int(subject_id_padded)}")
                print(f"{INDENT}You are now enrolled in {len(self.student.subjects)} out of 4 subjects")
                self._save()
                return
        print(f"{INDENT}Subject not found")

    def _show_subjects(self):
        print(f"{INDENT}Showing {len(self.student.subjects)} subjects")
        for subject in self.student.subjects:
            print(f"{INDENT}{subject}")

    def _change_password(self):
        print(f"{INDENT}Updating Password")
        while True:
            new_password = input(f"{INDENT}New Password: ").strip()
            if not validate_credentials(self.student.email, new_password):
                print(f"{INDENT}Incorrect password format")
                continue
            while True:
                ...
                if confirm == new_password:
                    ...
                    self._save()
                    break
                print(f"{INDENT}Password does not match - try again")
            break
